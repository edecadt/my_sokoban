...
static void my_print_hex_up_debut(unsigned long long *hex_up, va_list list,
    my_printf_t printf_struct)
{
    switch (printf_struct.length[0]) {
        case 'j':
            *hex_up = (intmax_t)va_arg(list, intmax_t);
            break;
        case 'z':
            *hex_up = (ssize_t)va_arg(list, ssize_t);
            break;
        case 'Z':
            *hex_up = (ssize_t)va_arg(list, ssize_t);
            break;
        case 't':
            *hex_up = (ptrdiff_t)va_arg(list, ptrdiff_t);
            break;
        default:
            *hex_up = va_arg(list, unsigned int);
            break;
    }
}

static void my_print_hex_up_final(unsigned long long *hex_up,
    va_list list, my_printf_t printf_struct)
{
    switch (printf_struct.length[0]) {
        case 'h':
            if (printf_struct.length[1] == 'h')
                *hex_up = (unsigned char)va_arg(list, unsigned int);
            else
                *hex_up = (unsigned short)va_arg(list, unsigned int);
            break;
        case 'l':
            if (printf_struct.length[1] == 'l')
                *hex_up = va_arg(list, unsigned long long);
            else
                *hex_up = va_arg(list, unsigned long);
            break;
        case 'q':
            *hex_up = (unsigned char)va_arg(list, unsigned int);
            break;
        default:
            my_print_hex_up_debut(hex_up, list, printf_struct);
    }
}
...
int my_print_hex_up(va_list list, my_printf_t printf_struct)
{
    ...
    my_print_hex_up_final(&number, list, printf_struct);
